...
La hipòtesi nul·la que estem intentant analitzar és que **el sorteig de la Loteria de Nadal és completament aleatori**, és a dir, que cada número té la mateixa probabilitat de ser premiat independentment de l'any. 

Per verificar-ho, hem estructurat l'anàlisi de la següent manera:
\newpage
...
Tot i que l'anàlisi principal del nostre estudi se centra en el període 2011-2025, hem trobat interessant observar la distribució històrica de l'última xifra del número guanyador de la Gross desde que aquesta es va començar a fer. 

Així podrem comprovar si hi ha algun patró al llarg dels més de 200 anys de sortejos. A continuació es mostra el gràfic de barres amb la freqüència de cada última xifra de la Grossa:
...
(No em deixa fer la taula més petita ns si a tu et surt)

Podem observar que la Grossa ha aparegut amb més freqüència amb les terminacions 5, 4, 8 i 0, en aquest ordre. Tot i que la probabilitat d’endur-se la Grossa és baixa, si el teu número coincideix en l’última xifra amb el premi, guanyaràs el reintegrament, assegurant-te així almenys de no perdre els diners invertits.
...
df %>%
  filter(!is.na(premio), categoria %in% top_categorias, is.na(letra)) %>%
  ggplot(aes(x = categoria, y = premio)) +
  geom_boxplot() +
  scale_y_log10() +
  labs(
    title = "Distribució del premi per categoria (Top 25 sense lletra)",
    x = "Categoria",
    y = "Premi (€)"
  ) +
  theme_minimal() +
...
df_digitos %>%
  ...
  scale_fill_gradient(low = "skyblue", high = "steelblue") +
  labs(
    title = "Freqüència dels dígits en els números de la Loteria",
    subtitle = "Anàlisi dels dígits de tots els números premiats",
    x = "Dígits",
    y = "Nombre d'aparicions"
  ) +
