...
Aquest treball presenta una anàlisi estadística del Sorteig de la Loteria de Nadal, l'esdeveniment de loteria més emblemàtic d'Espanya des de 1812. L'objectiu principal és avaluar l'homogeneïtat i l'aleatorietat del sistema mitjançant l'estudi d'un corpus de dades de 74270 registres (període 2011-2025) i la sèrie històrica de la Grossa (1812-2025). La metodologia s'ha centrat en la segmentació de les boles d'extracció directa respecte als premis derivats per càlcul, aplicant test de bondat d'ajust de Chi-quadrat de Pearson. Els resultats confirmen que el sistema és altament homogeni, validant la integritat mecànica dels bombos. L'estudi conclou que, malgrat percepcions popuars sobre "números de la sort", el sorteig es regeix per un atzar pur on l'esperança matemàtica es veu afectada negativament pel repartiment d'actius (70%) i la pressió fiscal.
...
Un dels mites més estesos en la Loteria de Nadal és la recerca de terminacions que "fa molts anys que no surten" sota la premissa que la sort "té un deute" amb elles. No obstant això, els resultats d'aquest estudi desmunten aquesta creença mitjançant dos pilars estadístics:

Independència de successos: Cada any, el bombo es buida completament i les 100.000 boles tornen a entrar en joc. Físicament, el sistema no té "memòria". El fet que el número 5 hagi sortit més vegades històricament no augmenta ni disminueix la seva probabilitat per al sorteig de l'any vinent.

La prova del p-valor (0,6403): El nostre test de Chi-quadrat sobre les 32.312 boles modernes és la prova definitiva. Un p-valor de 0,64 ens diu que si el sorteig fos perfectament aleatori, veuríem variacions com les actuals el 64% de les vegades.Això significa que les diferències de freqüència que veiem (que un número hagi sortit una mica més que un altre) són pures fluctuacions estadístiques (soroll) i no una tendència.

Si l'atzar tingués memòria i hagués de "compensar" els números que no surten, el nostre p-valor seria molt proper a 0 o 1, indicant un patró. En canvi, un p-valor tan neutre confirma que cada xifra té exactament un 10% de probabilitat cada desembre, independentment de si va sortir l'any passat o fa un segle. En resum, la "sort" no té deutes: el bombo comença de zero a cada sorteig.

L'esperança matemàtica
------------------------------------------------------------------------------------------------------------------------

Més enllà de la netedat del sorteig, és imprescindible analitzar el retorn esperat per al jugador. La Loteria de Nadal sovint es descriu com un "impost voluntari" o l'"impost a l'esperança", i les dades ho confirmen de manera clara quan comparem la inversió inicial amb el retorn estadístic mitjà.

Per determinar el valor real d'un dècim abans del sorteig, hem de considerar dos factors: el disseny del joc i l'impacte fiscal.

1. Esperança Brut (14,00 €):

L'esperança matemàtica bruta es calcula a partir del percentatge de la recaptació que es distribueix en premis. Segons el programa del sorteig, es destina el 70% de l'emissió a premis.

Cost del dècim: $20,00 \text{ €}$

Càlcul: $20,00 \text{ €} \times 0,70 = 14,00 \text{ €}$

Això significa que, d'entrada, l'Estat reté un 30% per a despeses de gestió i benefici públic.

2. Esperança Net (13,09 €):

L'esperança neta s'obté restant els impostos que l'Agència Tributària aplica als premis majors (aquells que superen els 40.000 €). En una sèrie de 100.000 números, l'impacte fiscal per dècim és:

Impost del Gordo (400.000 €): $(400.000 - 40.000) \times 0,20 = 72.000 \text{ €}$

Impost del 2n Premi (125.000 €): $(125.000 - 40.000) \times 0,20 = 17.000 \text{ €}$

Impost del 3r Premi (50.000 €): $(50.000 - 40.000) \times 0,20 = 2.000 \text{ €}$

Total impostos per sèrie: $72.000 + 17.000 + 2.000 = 91.000 \text{ €}$

Si dividim aquests 91.000 € d'impostos entre els 100.000 números que entren al bombo, obtenim una pressió fiscal mitjana de 0,91 € per dècim.

Càlcul: $14,00 \text{ € (Brut)} - 0,91 \text{ € (Impostos)} = 13,09 \text{ €}$

{r}
library(ggplot2)

df_retorn <- data.frame(
  Concepte = factor(c("Inversió (Dècim)", "Esperança Retorn (Brut)", "Retorn Net Estimat"), 
                    levels = c("Inversió (Dècim)", "Esperança Retorn (Brut)", "Retorn Net Estimat")),
  Valor = c(20, 14, 13,90)
)

ggplot(df_retorn, aes(x = Concepte, y = Valor, fill = Concepte)) +
  geom_col(width = 0.6, show.legend = FALSE) +
  geom_text(aes(label = paste0(Valor, " €")), vjust = -0.5, fontface = "bold", size = 4) +
  scale_fill_manual(values = c("#353436", "#2c7bb6", "#d7191c")) +
  scale_y_continuous(limits = c(0, 25)) + # Fixem el límit de l'eix y a 25
  labs(
    title = "Esperança Matemàtica de Retorn (per dècim)",
    subtitle = "Comparativa entre cost d'adquisició i valor estadístic real",
    x = "",
    y = "Import en Euros"
  ) +
  theme_minimal() +
...
L'anàlisi realitzada en aquest projecte permet concloure que el Sorteig de Nadal és un sistema robust que compleix estrictament les lleis de l'atzar. 

Mitjançant el test de Chi-quadrat, hem confirmat que no existeix cap biaix mecànic en les boles. Les diferències observades en les freqüències són estadísticament insignificants. Per tant, és impossible predir el número guanyador basant-se en sortejos anteriors.

Tot i que històricament les terminacions 5, 4 i 6 han estat les més premiades en la Grossa des de 1812, l'anàlisi del període modern (2011-2025) demostra que xifres com el 3 o el 0 també presenten una gran recurrència en els premis de bombo. Això confirma que qualsevol "patró" és fruit de la variabilitat natural de la mostra i no d'una propietat intrínseca dels números.

S'ha demostrat que l'anàlisi estadística només és vàlida si es separen les boles físiques dels premis derivats (terminacions, centenes i aproximacions). Aquests últims, en ser deterministes, distorsionen la percepció de l'atzar del jugador.

Des d'una perspectiva de consultoria, el sorteig es defineix com un joc de "summa negativa". L'esperança matemàtica de retorn és del 70%, que cau per sota del 60% en el cas dels grans premis degut al gravamen fiscal del 20%. La "sort" està repartida en una pluja fina de premis petits que fomenten la reinversió, però la rendibilitat a llarg termini per al jugador és inexistent.
\newpage
