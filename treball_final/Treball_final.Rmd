...
resum_global <- tibble(
  ...
  Valor = c(
    ...
    round(mean(df$premio, na.rm = TRUE), 2),
    resum_lletres
  )
)
...
resum_global %>%
  kable(
    caption = "Resum global de la Loteria de Nadal",
    format = "latex",
    booktabs = TRUE
  ) %>%
  kable_styling(font_size = 9, full_width = FALSE, bootstrap_options = c("striped", "hover"))
